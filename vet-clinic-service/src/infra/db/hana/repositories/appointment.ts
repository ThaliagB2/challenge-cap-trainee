...
export class AppointmentRepositoryImpl implements AppointmentRepository {
    ...
    public async findByIdAndDate(ids: string[], date?: Date): Promise<AppointmentModel[] | null> {
        let appointmentsQuery: cds.ql.ConstructedQuery<any> | cds.ql.ConstructedQuery<any>[];
        if (!date) appointmentsQuery = cds.ql.SELECT.from(this.APPOINTMENT).where({ id: { in: ids } });
        else appointmentsQuery = cds.ql.SELECT.from(this.APPOINTMENT).where({ id: { in: ids }, date: date });

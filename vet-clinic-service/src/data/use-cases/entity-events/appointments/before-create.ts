    ...
    public async execute(params: BeforeCreateAppointmentUseCase.Params): Promise<BeforeCreateAppointmentUseCase.Result> {
        try {
            ...
            if (checkOwnerValidity.hasError) {
                const errorMessages = checkOwnerValidity.errorMessages.map((error) => this.translator.translate(error)).join('\n');
                return left(new BadRequestError(errorMessages));
            }
            ...
            if (checkPetValidity.hasError) {
                const errorMessages = checkPetValidity.errorMessages.map((error) => this.translator.translate(error)).join('\n');
                return left(new BadRequestError(errorMessages));
            }
            ...
            if (checkVetValidity.hasError) {
                const errorMessages = checkVetValidity.errorMessages.map((error) => this.translator.translate(error)).join('\n');
                return left(new BadRequestError(errorMessages));
            }

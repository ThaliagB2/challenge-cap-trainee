...
import { CustomerModel, type CustomerResponse } from '@/domain/models/customer';
...
import { SalesOrderHeaderModel, type SalesOrderHeaderResponse } from '@/domain/models/sales-order-header';
...
| `private readonly BASE_URL` | Imports de `data/`, `presentation/` |
...
public async findAll(): Promise<Either<AbstractError, CustomerModel[]>> {
    // ← repositório usa throw; Either é papel da data layer
}
```
...
4. Erros são **lançados** (`throw`) — a data layer é responsável por convertê-los em `left()`.

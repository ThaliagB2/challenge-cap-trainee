...
> **Regra de isolamento:** a infra layer importa de `domain/` (para implementar interfaces) e de bibliotecas externas (`react`, `@mui/material`, `zustand`, `i18next`). **Não importa** de `data/`, `presentation/` ou `main/`.
...
2. **Errors são lançados (`throw`), não retornados como `Either`.** A data layer captura e converte para `left()`.
...
5. **Sem imports de `data/`, `presentation/` ou `main/`** dentro da infra.

...
import type { ThemeColorKey, ThemeConfig, ThemeLogoKey } from '@/infra/theme/theme-config';
import { DEFAULT_THEME } from '@/infra/theme/theme-defaults';
...
import { useThemeStore } from '@/infra/stores/theme-store';
...
❌ **Store importando de `data/` ou `presentation/`:**
```typescript
import { LoadThemeUseCase } from '@/data/use-cases/load-theme-use-case'; // ← proibido
```
...
6. Stores podem importar de `domain/` (tipos) e de `infra/theme/` — nunca de `data/`, `presentation/` ou `main/`.

...
> **Regra de isolamento:** `shared/` **não importa** de `domain/`, `data/`, `infra/`, `presentation/` ou `main/`. Importa apenas de pacotes externos genéricos (`dayjs`, `zod`, etc.). Tudo que precisa de uma camada específica não pertence ao `shared/`.
...
| Helpers de um use case específico | `data/use-cases/<feature>/` |

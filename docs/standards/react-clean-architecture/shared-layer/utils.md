...
## Quando uma função chega aqui

`shared/utils/` é o **topo** de uma escada de promoção, não o destino padrão de toda função pura. Função extraída de uma tela nasce local e sobe conforme aparecem consumidores reais:

| A função é usada por… | Onde vive |
|---|---|
| uma página (feature de página única) | `presentation/pages/<feature>/utils/` |
| uma página (feature multi-página) | `presentation/pages/<feature>/<page>/utils/` |
| 2+ páginas da mesma feature | `presentation/pages/<feature>/utils/` |
| um componente global (é predicado *dele*) | `.ts` ao lado do componente, exportado pelo barrel |
| 2+ features, sem dependência de camada | **`shared/utils/`** |

A pasta se chama `utils/` nos três degraus de propósito: promover é mudança de caminho, não de conceito.

> **Condição de entrada:** a shared layer **não importa de nenhuma outra camada** — nem de `domain/`. Uma função tipada contra um model (`(order: SalesOrderModel) => …`) **não pode** subir para cá. Se ela é usada por 2+ features mesmo assim, mora num `.ts` ao lado do componente global a que pertence e sai pelo barrel; a alternativa é generalizá-la para receber primitivos. Bibliotecas externas (`dayjs`, tipos do MUI) são permitidas.

Promova no **segundo consumidor real**, nunca por antecipação. Função genérica na aparência mas com um consumidor só fica onde está.
...
5. `shared/utils/` **não importa de nenhuma camada** do projeto — só libs externas. O que depende de `domain/` para no nível da feature.
6. Chega aqui por **promoção**, no segundo consumidor de outra feature — não por antecipação.

...
Aqui vivem: `interface XxxRepository`, `interface XxxUseCase`, `class XxxModel`, a hierarquia de erros (`AbstractError` + subclasses HTTP) e os protocolos técnicos que o domínio precisa (`interface HttpClient`). Toda implementação concreta vive em `infra/` ou `data/`; toda DI acontece em `main/factories/`.

> **Regra de isolamento (a mais importante da arquitetura):** a domain layer **não importa nada** de `data/`, `infra/`, `presentation/`, `main/`, `shared/` nem de qualquer framework (`react`, `@mui/material`, `zustand`, `i18next`, `react-router-dom`). Dentro de `domain/`, módulos podem se referenciar livremente. O único pacote externo aceito é `@sweet-monads/either` para o tipo `Either<L, R>`.
...
2. **`Either<AbstractError, T>` é o tipo canônico de `Result` em use cases.** Importado direto de `@sweet-monads/either`. Quem produz `left()`/`right()` é a data layer; o domain só declara o tipo.
...
| `services/` | Não existe service no domain React — lógica vai para `data/use-cases/` | `data/use-cases/<feature>/` |
...
| `data/` | ✅ Implementações consomem contratos do domain |
...
|---|---|
| `data/`, `infra/`, `presentation/`, `main/`, `shared/` | ❌ Nunca |

...
Use cases no domain são **contratos de operações de negócio**. Cada interface declara o que pode ser executado; a implementação concreta fica em `data/use-cases/`. O tipo de retorno é sempre `Either<AbstractError, T>` — nunca `throw`, nunca `Promise<T>` sem o wrapper.
...
### Exemplo 4 — Implementação correspondente na data layer (referência)
...
| `Either<AbstractError, T>` como `Result` | Imports de `infra/`, `react`, `data/` |

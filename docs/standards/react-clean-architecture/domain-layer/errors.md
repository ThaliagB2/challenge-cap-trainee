...
| `data/use-cases/` | **Captura** e retorna via `left(new ServerError(...))` |
...
// 2. data: captura e retorna como Either — preserva o tipo do erro de domínio
...
5. Erros são lançados na `infra/` e **retornados** (via `Either`) pela `data/` — nunca relançados pela `presentation/`.

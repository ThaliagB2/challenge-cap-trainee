...
O arquivo da factory usa kebab-case **sem sufixo** (`load-customers.ts`), espelhando o nome do use case correspondente em `data/use-cases/` (que mantém o sufixo `-use-case.ts`).
...
import { CreateSalesOrderUseCase } from '@/data/use-cases/sales-orders/create-sales-order-use-case';

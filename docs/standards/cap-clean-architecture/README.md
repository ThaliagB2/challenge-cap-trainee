...
> **Adaptabilidade non-CAP:** este padrão é a referência canônica para Clean Architecture na Numen DS, escrito para SAP CAP. Para backends Node puro (Fastify, Express, Hono) sem CDS/HANA, a arquitetura (5 camadas, dependency rule, `Either`, models ricos, DI manual) permanece idêntica — apenas os artefatos CDS são substituídos por equivalentes Node. Ver [ADR-001](../../adr/001-adaptacao-cap-clean-architecture-para-backend-node-puro-fastify.md) para o mapeamento completo CDS → Fastify/Drizzle.
